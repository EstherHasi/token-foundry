...
```bash
python skills/token-audit/scripts/tokenkit.py ingest tokens.css -o work/model.json
python skills/token-audit/scripts/tokenkit.py audit work/model.json -o work
python skills/token-audit/scripts/tokenkit.py generate work/model.json -o out
```
Being tokens.css the tokens file you want to audit.
work/model.json is generated by the plugin on your work-folder
The plugin generates also an audit.md file, where you can read the complete audit report
In the out folder you will find the new generated tokens.css and the tokens.manifest.json to import in Figma with the plugin in the export tab you can include or just give it the file to Claude and it'll take care of the rest, as the plugin writes the Figma IDs of the variables.
